 ...
 */
 ...
 void geoInitialize(femGeo *theGeometry) {
   ...
   theGeometry->nodeOrder = NULL;
   theGeometry->nodeInv   = NULL;
 }
 ...
 void geoFree(femGeo *theGeometry) {
   ...
   /* libération du mapping */
   if (theGeometry->nodeOrder) free(theGeometry->nodeOrder);
   if (theGeometry->nodeInv)   free(theGeometry->nodeInv);
   free(theGeometry);
 }
 ...
 void geoMeshWrite(femGeo* theGeometry, const char *filename) 
 {
    FILE* file = fopen(filename,"w");
